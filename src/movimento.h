...
public:
	...
	Movimento(enderecoPorta ladoEsquerdo, enderecoPorta ladoDireito){
		motorEsquerdo = new Motor(ladoEsquerdo, Motor::motorGrande);
		motorDireito = new Motor(ladoDireito, Motor::motorGrande);
		motorEsquerdo->reset();
		motorDireito->reset();
	}
	...
	void setVelocidadeIndividual(int velocidadeDireita, int velocidadeEsquerda, int modo = Movimento::modoGraus){
		if(modo==Movimento::modoGraus){
			motorDireito->setVelocidadeDesejada(velocidadeDireita);
			motorEsquerdo->setVelocidadeDesejada(velocidadeEsquerda);
		}else if(modo==Movimento::modoCentimetros){
			//envia o valor correto para centimetros por segundo
			motorDireito->setVelocidadeDesejada(velocidadeDireita/getDistanciaPorGrau());
			motorEsquerdo->setVelocidadeDesejada(velocidadeEsquerda/getDistanciaPorGrau());
		}
	}

	void vaiPraFrente(){
		motorDireito->giraParaSempre();
		motorEsquerdo->giraParaSempre();
	}
	void vai(){
		motorDireito->giraParaSempre();
		motorEsquerdo->giraParaSempre();
	...
	void parar(){
		motorDireito->setVelocidadeDesejada(0);
		motorEsquerdo->setVelocidadeDesejada(0);
		motorEsquerdo->parar();
		motorDireito->parar();
		esperarParar();
	}
